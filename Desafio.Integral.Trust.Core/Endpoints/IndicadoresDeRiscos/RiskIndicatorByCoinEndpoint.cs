...
public class RiskIndicatorByCoinEndpoint : IEndpoint
{
    public static void Map(IEndpointRouteBuilder app)
        => app.MapPost("/coin", HandleAsync)
            .WithName("Indicadores: Total por moeda")
            .WithSummary("Relata Indicador: Total por moeda")
            .WithDescription("Relata Indicador: Total por moeda")
            .WithOrder(1)
            .Produces<Response<IndicadorDeRisco?>>();

    private static async Task<IResult> HandleAsync(
        ClaimsPrincipal user, IIndicadoresRiscoHandler handler, GetRiskIndicatorByCoin request)
    {
        request.UserId = user.Identity?.Name ?? string.Empty;
        var result = await handler.ValorTotalTransacoesPorMoeda(request);
